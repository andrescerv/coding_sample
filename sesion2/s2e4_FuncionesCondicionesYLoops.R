# Sesión 2, ejemplo 4: Funciones, condiciones y loops

# OBJETIVO
  # Aprender a hacer un loop con IF ELSE

# DESARROLLO
  # Vamos a crear una un funcion para validar si nuestra variable es superior a 40

variable <- 8

if (variable > 40){
 print("La variable es superior a 40")
} else {
 print ("La variable no es superior a 40")
}

# Alternativamente, podemos usar la funcion ifelse, 
  # que nos ayudara a plantear el resultado a obtener si se cumple la condicion, 
  # y a continuacion el resultado a obtener si no se cumple la misma

ifelse(variable>40, "La variable es superior a 40", "La variable no es superior a 40") 
